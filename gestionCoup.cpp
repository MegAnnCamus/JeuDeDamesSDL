...
int getPossibleCasePos(int c, int diag, plateau p) {
	...
	switch (diag) {
	case 1:
		while (x > 0 && y > 0) { //DIAGONALE HAUT GAUCHE
			x--;
			y--;
		...
		break;
	case 2:
		while (x <= 10 && y > 0) { //DIAGONALE HAUT DROITE
			x++;
			y--;
			casePlateau aPrendre = getCasePlateau(x, y, p);
			if (!aPrendre.isLibre) { /* si on trouve une case non libre */
		...
		break;
	case 3:
		while (x > 0 && y <= 10) { //DIAGONALE BAS GAUCHE
			x--;
			y++;
			casePlateau aPrendre = getCasePlateau(x, y, p);
			if (!aPrendre.isLibre) { /* si on trouve une case non libre */
		...
		break;
	case 4:
		while (x <= 10 && y <= 10) { //DIAGONALE BAS DROITE
			x++;
			y++;
			casePlateau aPrendre = getCasePlateau(x, y, p);
			if (!aPrendre.isLibre) { /* si on trouve une case non libre */
...
coup* coupsPossiblesDames(const casePlateau c, const plateau p) {
	...
	while (fini==FALSE) {
		...
		if (casePrise==FALSE || caseDerriere.isLibre==FALSE
			...
			int x = c.x - 1;
			int y = c.y - 1;
			while (x > 0 && y > 0 && getCasePlateau(x, y, p).isLibre==TRUE) { //on parcourt la diagonale jusqu'au bout ou jusqu'a rencontrer un autre pion.
				casePlateau newCase = getCasePlateau(x, y, p);
				if (newCase.notation != 0) {
					...
					cp.nbPrises = 0;
					cp.newCase = newCase.notation;
					cps[nbCoup++] = cp; //on ajoute tous les coups a la liste
				}
				x--;
				y--;
			}
			...
			int x = cPrise.x - 1;
			int y = cPrise.y - 1;
			while (x > 0 && y > 0 && getCasePlateau(x, y, p).isLibre==TRUE) { //dans ce cas, on parcourt les cases de la diagonale derriere la prise
				coup cp;
				cp.type = PRISE;
				cp.oldCase = c.notation;
				cp.nbPrises = 1;
				...
				if (getPossibleCasePos(newCase.notation, 2,
						...
						newCase.notation, 3, finalBoard)) { //si il y a une prise possible sur la diagonale, on l'ajoute a la liste
					coup * coupsCompletes =
							completerCoupDame(cp, 1, finalBoard);
		...
		if (casePrise==FALSE || caseDerriere.isLibre==FALSE
			...
			int x = c.x + 1;
			int y = c.y + 1;
			while (x <= 10 && y <= 10 && getCasePlateau(x, y, p).isLibre==TRUE) { //on parcourt la diagonale jusqu'au bout ou jusqu'a rencontrer un autre pion
				casePlateau newCase = getCasePlateau(x, y, p);
				if (newCase.notation != 0) {
					coup cp;
					cp.type = '-';
